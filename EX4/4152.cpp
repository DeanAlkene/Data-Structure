...
int main()
{
    ...
    upper[strlen(n) / 2 + 1] = '2';
    upper[strlen(n) / 2 + 2] = '\0';
    bigInt num(n), left("1"), right(upper), two("2"), one("1"), count, mid, val;
    ...
    if(left.isOdd())
        cout << n - count << '/' << left + two + count - n << '\n';
    else
        cout << left + two + count - n << '/' << n - count << '\n';
